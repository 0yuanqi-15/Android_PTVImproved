...
public class NearStopListAdapter extends ArrayAdapter<Stop> {
    ...
    public NearStopListAdapter(@NonNull Context context, int resource, @NonNull ArrayList<Stop> objects) {
        ...
        timeSingleton = Time.getInstance();
...
//        return super.getView(position, convertView, parent);
        ...
        if (routes.size()>=1){
            ...
            String time = routes.get(0).equals("--") ? "--:--" : timeSingleton.gapInString(times.get(0));
            tvtime1.setText(time);
        }

        if (routes.size()>=2){
            tvroute2.setText(routes.get(1));
            String time = routes.get(1).equals("--") ? "--:--" : timeSingleton.gapInString(times.get(1));
            tvtime2.setText(time);
        }

        if (routes.size()>=3){
            tvroute3.setText(routes.get(2));
            String time = routes.get(2).equals("--") ? "--:--" : timeSingleton.gapInString(times.get(2));
